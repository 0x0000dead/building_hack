...
podpr_bl = col[1].text_input('Подпрограмма', value='Общеобразовательные учреждения')
uniq_id = col[2].text_input('Уникальный айди', value='022-0527')
identificator = col[3].text_input('Идентификатор вида', value=1.1)
name_task = col[4].text_input('Название задачи', value='Предпроектные работы')
percent_end = col[5].text_input('Процент завершения', value=100.0)
begin = col1[0].text_input('Дата начала задачи', value="2022-01-14")
end = col1[1].text_input('Дата окончания задачи', value="2023-03-30")
begin_bp = col1[2].text_input('Дата начала БП0', value="2021-05-04")
end_bm = col1[3].text_input('Дата окончания БП0', value="2021-05-04")
status_exp = col1[4].text_input('Статус по экспертизе', value=1.0) # maybe -1 is none, or empty cell
form_ksg = col1[5].text_input('Дата формирования КСГ', value="2023.01.17") # "-" - meaning none
expertize = col2[0].selectbox('Экспертиза', ["1ТС", "2ТС","1Т","1С","2С","2Т","-"], index=2)
code_ds = col2[1].text_input('Код ДС', value='022-0043')
status_sq = col2[2].selectbox('состояние площадки', ["Свободна, передана", 
                                                               "Занята",
...
name_of_worker = col2[5].text_input('Кол-во рабочих', value='wagondigitalnewyearrailway:)happy')
date_rep = col2[6].text_input('date_report', value='2023.01.20')
